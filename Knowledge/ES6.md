...
- Promise.any()（ES2021）：有一个成功即返回，所有失败才返回失败。
...
#### 例题
``` TS
// 当 new Promise() 被创建时，Promise 内部的执行函数（(resolve, reject) => {...}）立即执行。
var promise = new Promise((resolve, reject) => { console.log(1) }) 
// 这里的 then() 传入的回调是 console.log(2)，实际上会立即执行并输出 2，然后将其返回给 then()，这并不会等到 Promise 的状态改变后再执行。
...
```
``` TS
Promise.resolve(1) // 返回一个状态为 fulfilled 且值为 1 的 Promise
.then(2) // 传给 .then 的参数是数字 2，不是一个函数,会发生 Promise 穿透————这个 .then 被忽略了，它会将上一个 Promise 的结果（即 1）原封不动地传递给下一个环节。
.then(Promise.resolve(3)) // 参数是一个 Promise 对象，再次穿透
.then(console.log) // 1被传入
// 1
```
``` TS
async function async1() {
    ...
    await async2()
    console.log('async1 end') // 放入微队列 1
}
