...
#### 浅拷贝
- 定义：只复制对象的第一层属性，如果属性是引用类型（对象/数组），只会复制引用地址。不会开辟新的内存空间，共享内存空间。基本数据类型直接拷贝
...
- 作用域分类：
 ...
 - 作用域链定义：JS官方用来**查找变量的机制**，当访问一个变量的时候，JS 引擎会从**当前作用域**开始查找，如果找不到就沿着**父级作用域链向上查找**，直到全局作用域。
...
- **原型（对象属性共享机制）**：Object.prototype
  ...
  - 显示原型(函数才有)：`prototype`，隐式原型（对象才有）：`__proto__`
  - 原型上有个 `constructor` 属性，指向关联的构造函数
